...
function requireDatabase(req, res, next) {
  if (!isDbEnabled) {
    res.status(503).json({
      error: 'Quiz library database is unavailable because DATABASE_URL is not configured.',
...
router.get('/quiz-topics', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Could not load quiz topics.' });
  }
});
...
router.post('/quiz-topics', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Could not create quiz topic.' });
  }
});
...
router.put('/quiz-topics/:topicId', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Could not update quiz topic.' });
  }
});
...
router.delete('/quiz-topics/:topicId', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Could not delete quiz topic.' });
  }
});
...
router.post('/quiz-topics/:topicId/questions', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Could not create quiz question.' });
  }
});
