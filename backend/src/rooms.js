...
export function addStudent(joinCode, socketId, nickname = 'Student') {
  const room = getRoom(joinCode);
  if (!room) return null;

  const cleanNickname = String(nickname || 'Student').trim() || 'Student';
...
export function updateRoomState(joinCode, patch = {}) {
  ...
  const allowedKeys = ['dim', 'concept', 'A', 'v', 'u', 'alpha', 'beta', 't', 'animSpeed', 'camera3D'];
  const cleanPatch = {};
