const presets2D = {
  identity: [[1, 0], [0, 1]],
  scale: [[1.5, 0], [0, 1.5]],
  rotate: [[0.71, -0.71], [0.71, 0.71]],
  shear: [[1, 1], [0, 1]],
  reflect: [[1, 0], [0, -1]],
  collapse: [[1, 1], [2, 2]],
};

const presets3D = {
  identity: [[1, 0, 0], [0, 1, 0], [0, 0, 1]],
  scale: [[1.5, 0, 0], [0, 1.5, 0], [0, 0, 1.5]],
  rotate: [[0.71, -0.71, 0], [0.71, 0.71, 0], [0, 0, 1]],
  shear: [[1, 1, 0], [0, 1, 0], [0, 0, 1]],
  reflect: [[1, 0, 0], [0, 1, 0], [0, 0, -1]],
  collapse: [[1, 0, 1], [0, 1, 1], [1, 1, 2]],
};

function updateCell(A, row, col, value) {
  return A.map((r, i) => r.map((cell, j) => (i === row && j === col ? Number(value) : cell)));
}

export default function MatrixInput({ dim, A, onChange }) {
  const presets = dim === 3 ? presets3D : presets2D;

  return (
    <div className="card-section">
      <div className="section-title">
        Matrix A
        <span id="matrixSizeLbl" style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: 'var(--text-subtle)', textTransform: 'none', letterSpacing: 0 }}>
          {dim}×{dim}
        </span>
      </div>

      <div className="matrix-grid">
        {A.map((row, rowIndex) =>
          row.map((value, colIndex) => (
            <input
              key={`${rowIndex}-${colIndex}`}
              className={`matrix-cell ${dim === 2 && colIndex === 2 ? 'd3-only' : ''}`}
              type="number"
              step="0.1"
              value={value}
              onChange={(e) => onChange(updateCell(A, rowIndex, colIndex, e.target.value))}
            />
          ))
        )}
      </div>

      <div className="preset-grid">
        {Object.keys(presets).map((name) => (
          <button key={name} className="preset-btn" onClick={() => onChange(presets[name])}>
            {name.charAt(0).toUpperCase() + name.slice(1)}
          </button>
        ))}
      </div>
    </div>
  );
}
